...
public class ElasticsearchTool {
    ...
    private static List<Map<String, Object>> setSearchResponse(SearchResponse searchResponse, String highlightField) {
        ...
        for (SearchHit searchHit : searchResponse.getHits().getHits()) {
            searchHit.sourceAsMap().put("id", searchHit.getId());

            if (StringUtils.isNotEmpty(highlightField)) {

                System.out.println("遍历 高亮结果集，覆盖 正常结果集" + searchHit.sourceAsMap());
                ...
                if (text != null) {
                    ...
                    //遍历 高亮结果集，覆盖 正常结果集
                    searchHit.sourceAsMap().put(highlightField, stringBuffer.toString());
                }
            }
            sourceList.add(searchHit.sourceAsMap());
        }
